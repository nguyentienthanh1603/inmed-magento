 ...
 */
class Magestore_Customerattribute_Block_Adminhtml_Managecustomer_Edit_Tab_Form 
extends Mage_Adminhtml_Block_Widget_Form implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
		...
		$tabs = ['group', 'consumer', 'clinician', 'avatar'];

		foreach ( $tabs AS $tab )
		{
			$customerAttributes = $this->getCustomtabInfo($tab);
			$customer = $this->getCustomerAttribute();


			$fieldset = $form->addFieldset('customerattribute_form_'.$tab, array(
				'legend'=>Mage::helper('customerattribute')->__(ucwords($tab))
			));
...
//		foreach($customerAttributes as $customerAttribute)
//		{
//			echo $customerAttribute->getCustomTab();
//		}
//		exit();
//
